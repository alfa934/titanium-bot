...
ros::Publisher pub_motor, pub_pid_y, pub_pid_w, pub_base;
...
void timer10msCallback(const ros::TimerEvent &event)
{
    ...
    switch (robot_state)
    {
        ...
        case 2:
            ...
            if(controller.crs) //--- this should be Circle button since it moves side-side
            {
                robot_state++;
            }
            break;
        case 3:
        {
            ...
            if((!left_lost && !right_lost) && controller.down) //--- this should be X button since it does the homing WHILE the arm takes something
            {
                base_ready.data = true;
                robot_state = 1;
            }
            else
            {
                base_ready.data = false;
            }
