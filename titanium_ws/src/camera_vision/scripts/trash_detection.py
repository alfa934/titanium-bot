...
from robot_msgs.msg import camera
from ultralytics import YOLO
import pickle  # Added for loading calibration data
...
class TrashDetector:
    def __init__(self):
        ...
        rospy.loginfo("Loading camera calibration data...")
        try:
            with open("/home/roobics/titanium-bot/titanium_ws/src/camera_vision/scripts/calibration.pkl", "rb") as f:
            ...
            self.mapx, self.mapy = cv2.initUndistortRectifyMap(
                camera_matrix, dist_coeffs, None, self.new_camera_matrix, (w, h), cv2.CV_32FC1)
            
            rospy.loginfo("Camera calibration data loaded successfully")
        except Exception as e:
            rospy.logerr(f"Failed to load calibration data: {e}")
            rospy.signal_shutdown("Calibration data loading failed")
            return
    ...
    def infer(self):
        ...
        # Apply camera calibration (undistort the frame)
